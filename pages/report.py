...
from pages.overview import show_overview_table, show_alerts, show_dtype_df, missing_values_plot, show_outliers
from pages.visualization import categorical_format, numerical_format, timeseries, string
from pages.correlation import get_pairplot, normality_test, plot_heatmap
from pages.modeling import plot_feature_importances, show_all_results
...
if 'report_page' not in st.session_state:
    pages = ["Overview", "Alert", "Data Types", "Missing Values", "Outlier", 
             "Categorical", "Boolean", "Discrete", "Continuous", "Time Series", 
             "String", "Pairplot", "Normality", "Correlation Heatmap", "Modeling"]
    st.session_state.report_page = pd.DataFrame({
        "Page": pages,
        "Select": [False] * len(pages)
    })
...
# sidebar
st.sidebar.markdown(_("**Select the page you want to add**"))
set_report()